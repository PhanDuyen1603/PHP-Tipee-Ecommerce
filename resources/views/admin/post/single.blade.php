...
if(isset($post_detail)){
    ...
    $thumbnail = $post_detail->thubnail;
    $thumbnail_alt = $post_detail->thubnail_alt;
    ...
    $data_cats=\Illuminate\Support\Facades\DB::table('category')
              ->join('join_category_post','category.categoryID','=','join_category_post.id_category')
              ->where('join_category_post.id_post', $sid)
              ->select('category.categoryName','category.categorySlug','category.categoryID')
              ->first();
    $link_url_check="";
    if($data_cats):
        $slug_cat=$data_cats->categorySlug;
        $link_url_check=route('tintuc.details',array($slug_cat,$post_detail->slug));
    endif;
} else{
    $title = 'Viết bài mới';
    $post_title = "";
    	    ...
    	    <div class="row">
                ...
                <div class="col-3">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <?php
                                $list_cate = App\Model\Category::orderBy('category.categoryName', 'ASC')->select('category.categoryName', 'category.categoryID')->get(); 
                                if($sid != 0){
                                    $data_checks = App\Model\Join_Category_Post::where('id_post', $post_detail->id)->get();
                                }
                            ?>
