...
if($categories->seo_title !=""):
	...
	$title=Helpers::get_option_minhnn('seo-title-add');
endif;
...
if($categories->thubnail !=""):
	$thumb_img_seo=url('/images/category/')."/".$categories->thubnail;
else:
...
$data_seo = array(
    ...
    'current_url_amp' => route('amp.category.list',array($categories->categorySlug))
);
        ...
        <div class="container clear">
            <div class="breadcrumbs_top_page clear">
                <div class="breadcrumbs-item fl">
                    {!! Breadcrumbs::render('tintuc.category',$categories) !!}
                </div>
                    ...
                    <div class="container_category_top_page clear">
                        ...
                        <div class="container_right">
                            <div class="content_slider_group_home clear">
                              <?php
                                    $thumbnail_src_category="";
                                    $thumbnail_alt_category=(isset($categories->thubnail_alt) || $categories->thubnail_alt !="")?$categories->thubnail_alt :'banner category';
                                    if(isset($categories->thubnail) && $categories->thubnail !=""):
                                        $thumbnail_src_category=url('/')."/images/category/".$categories->thubnail;
                                    else:
                                        $thumbnail_src_category=url('/').'/img/danh_muc_default.jpg';
                                    endif;
                               ?>
                                <div class="category_thumbnail_feature clear">
                                    <div class="thumbnail_cate">
                                        <img src="{{$thumbnail_src_category}}" alt="{{$thumbnail_alt_category}}"/>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="row-content-category clear">
                                <div class="listProduct box clear">
                                    <div class="container_theme_category">
                                        <h1 class="title_product"><span>{{$categories->categoryName}}</span></h1>
                                        <input type="hidden" class="cate-slug" name="cate-slug" value="{{$categories->categorySlug}}">
                                        @if(WebService::get_child_category_product($categories->categoryID,5))
                                            <ul class="list_child clear">
                                            @foreach(WebService::get_child_category_product($categories->categoryID,5) as $listChildCategories)
                                               <li><a href="{{route('category.list',$listChildCategories->categorySlug)}}">{{$listChildCategories->categoryName}}</a></li>
                                            @endforeach
                                            </ul>
                                        @endif
                                    </div>
                                    <div class="clear">
                                      <div class="category_filter fr">
                                        Sắp xếp: <select name="filter_cate">
                                          <option value="">Mặc định</option>
                                          <option value="pricea" <?php if(isset($_GET['ordersort']) && $_GET['ordersort'] == 'pricea'){ echo 'selected'; } ?> >Giá tăng dần</option>
                                          <option value="priced" <?php if(isset($_GET['ordersort']) && $_GET['ordersort'] == 'priced'){ echo 'selected'; } ?>>Giá giảm dần</option>
                                        </select>
                                      </div>
                                    </div>
                  									@if($categories->categoryDescription !="")
                                      <div class="panel-bxslider-tab cate_box clear">
                                            <div class="cate_desc clear">
                                                   <div class="cate_desc_content clear">
                                                       <?php echo htmlspecialchars_decode($categories->categoryDescription);?>
                                    ...
                                    <div class="list_theme_category product_category_container clear">
                                        @if(count($data_customers)>0)
                                            <div class="row row-listpro flex wrap">
                                                ...
                                                ?>
                                              <div id="theme_cate_{{$k}}" class="item_product_list product_item_list col col-lg-5ths item-page-category-product" data-open="0" data-id="{{$data_customer->id}}" data-parent="{{$categories->categoryParent}}" data-cate="{{$id_category}}">
                                    ...
                                    @if($categories->categoryContent !="")
                                         <div class="panel-bxslider-tab panel-tab-content-seo cate_box clear">
                                            <div class="details-sumary clear">
                                                 <div class="entry_content clear">
                                                     <?php echo htmlspecialchars_decode($categories->categoryContent);?>
                                                 </div><!--cate_desc_content-->
                                            </div><!--cate_desc-->
                                         </div><!--panel-bxslider-tab-->
                                    @endif
